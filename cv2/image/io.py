...
from ..ascend.common.path import check_file_exist, is_str, mkdir_or_exist
...
def _pillow2array(img, flag='color', channel_order='bgr'):
    ...
    if flag == 'unchanged':
        ...
        if img.mode != 'RGB':
            if img.mode != 'LA':
                # Most format except 'LA' can be directly converted to RGB
                img = img.convert('RGB')
            else:
...
def imread(img_or_path, flag='color', channel_order='bgr', backend=None):
    ...
    Args:
        ...
        backend (str | None): The image decoding backend type. Options are
            `cv2`, `pillow`, `None`. If backend is None, the
